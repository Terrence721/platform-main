...
describe('EntitySelectors$', () => {
  ...
  describe('EntitySelectors$Factory.create (Hero)', () => {
    ...
    beforeEach(() => {
      ...
      // Store's constructor expects a StateObservable ({ state$, state }),
      // not a raw Observable directly - state$ (the BehaviorSubject) alone
      // no longer satisfies it now that Store composes rather than extends
      // Observable (see store/src/state.ts). No test here reads the signal.
      store = new Store<{ entityCache: EntityCache }>(
        { state$, state: (() => state$.value) as any } as StateObservable,
        null as any,
        null as any
      );
