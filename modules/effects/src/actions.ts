...
export class Actions<V = Action> extends Observable<V> {
  constructor(
    ...
    source?: Observable<V> | ScannedActionsSubject
  ) {
    super();

    if (source) {
      this.source = (
        source instanceof ScannedActionsSubject ? source.asObservable() : source
      ) as Observable<V>;
    }
  }
 ...
 *
 * For example, if `actions` has type `Actions<AdditionAction|SubtractionAction>`, and
